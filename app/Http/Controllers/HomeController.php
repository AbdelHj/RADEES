...
public static function getEmployees($email)
{
    // Récupération de la liste des départements
    $deps = Dep::all();
    ...
    $ret = 0;
    if ($email) {
        $ret = $employeesQuery->where('email', $email)->exists();
    }
...
public function importuser(Request $request)
{
    ...
    print_r($import);
    if(EmployeesImport::$ret == 0){
        return view('error',compact('file'));
    }
    EmployeesImport::$ret = 0;
    EmployeesImport::$total = 0;
